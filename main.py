...
@bot.command(name="assign", description="Assigns a moodle course with a textchannel", scope=GUILD_ID,
             options=[
                 interactions.Option(name="text_channel", description="choose textchannel",
                                     type=interactions.OptionType.CHANNEL, required=True),
...
async def assign(ctx: interactions.CommandContext, text_channel: interactions.Channel):
    ...
    if options:
        ...
        if text_channel.type == interactions.ChannelType.GUILD_TEXT:
            await ctx.send(f"Select a course that you like to assign to textchannel <#{text_channel.id}>",
                           components=ActionRow.new(SelectMenu(custom_id="assign_selector", options=options, )))
        else:
...
async def uploadFile(path):
    SCOPES = ['https://www.googleapis.com/auth/docs', 'https://www.googleapis.com/auth/drive',
              'https://www.googleapis.com/auth/drive.metadata.readonly', '/auth/docs']
...
async def run_task():
    ...
    if not exists("moodle/"):
        os.mkdir("moodle/")
        print(colored("Ordner moodle/ wurde erstellt. Bitte führe die Konfiguration von dem Moodle-Downloader aus", 'green'))
        exit()
            ...
            if rfo[0] >= 1:
                ...
                if ch_con:
                    ...
                    for ccon in ch_con.fetchall():
                        print(ccon[0])
                        query = f'SELECT saved_to, content_filesize, course_id, section_name, section_id FROM files WHERE time_stamp >= {ptime} AND ' + generateFilter(
                            ['php', 'html', 'css', 'md']) + f'course_id={ccon[0]} ORDER BY section_id'
                        ...
                        if channel:
                            if channel.type in [interactions.ChannelType.GUILD_TEXT,
                        ...
                        for key in files:
                            ...
                            if thread_res:
                                channel = await interactions.get(client=bot, obj=interactions.Thread,
                            ...
                            if files.get(key):
                                if channel.type != interactions.ChannelType.PUBLIC_THREAD:
                                    ...
                                    try:
                                        ...
                                        time.sleep(1)
                                        thread: interactions.Channel = await channel.create_thread(name=key,
