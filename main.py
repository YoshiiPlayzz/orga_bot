...
async def fetchChannels():
    ...
    if res: 
        for v in res.fetchall():
            channel = await interactions.get(client=bot, obj=interactions.Channel, object_id=v[1])
...
async def on_ready():
    ...
    res = cur.execute("CREATE TABLE IF NOT EXISTS course_channel_assignment(course_id INT, text_channel INT, PRIMARY KEY(course_id, text_channel), FOREIGN KEY(course_id) REFERENCES files(course_id))")
...
async def assign(ctx: interactions.CommandContext, text_channel: interactions.Channel):
    ...
    if options:
        ...
        if text_channel.type == interactions.ChannelType.GUILD_TEXT:
            await ctx.send(f"Select a course that you like to assign to textchannel <#{text_channel.id}>", components=ActionRow.new(SelectMenu(custom_id="assign_selector",options=options,)))
        else:
...
async def generateOptions(text_ids: interactions.Snowflake):
    ...
    for item in courses: 
        ...
        options.append(SelectOption(label=f"{item}", value=f"{course_ids[p]};{text_ids};{random.random()}"))
        p = p+1
    return options
...
async def modal_response(ctx: interactions.ComponentContext, response):
    if response:
        ...
        r = cur.execute(f"SELECT COUNT(*) FROM course_channel_assignment WHERE course_id={id}")
     
        if r.fetchone()[0] == 0:
            res = cur.execute(f"INSERT INTO course_channel_assignment VALUES({id}, {text})")
            con.commit()
            r2 = cur.execute(f"SELECT COUNT(*) FROM course_channel_assignment WHERE course_id={id}")
...
def generateFilter(filter: List[str]): 
     s = ""
     for a in filter:
        s = s + f"(content_filename NOT LIKE '%.{a}' AND saved_to NOT LIKE '%.{a}') AND "
     return s
...
async def uploadFile(path):
    ...
    # TODO: Check if user created the file token.json
            ...
            if rfo[0] >= 1:
                   ...
                   if ch_con:
                    ...
                    for ccon in ch_con.fetchall():
                        print(ccon[0])
                        query = f'SELECT saved_to, content_filesize, course_id, section_name, section_id FROM files WHERE time_stamp >= {ptime} AND ' + generateFilter(['php', 'html', 'css', 'md']) + f'course_id={ccon[0]} ORDER BY section_id'
                        ...
                        if channel:
                            if channel.type in [interactions.ChannelType.GUILD_TEXT, interactions.ChannelType.PUBLIC_THREAD]:
                                for re in res2.fetchall():
                                    if re[1] >= 8389000000:
                                        ...
                                        if (os.path.getsize(re[0]) / (1024*1024)) <= 8:
                                            ar.append(file)
                        ...
                        for key in files:
                            ...
                            req = cur.execute(f"SELECT thread_channel.thread_id FROM thread_channel, files WHERE files.section_name='{key}' and files.course_id='{ccon[0]}' and files.section_id=thread_channel.section_id")
                            thread_res = req.fetchone()
                            if thread_res:
                                channel = await interactions.get(client=bot, obj=interactions.Thread, object_id=thread_res[0])
                            if files.get(key):
                                if channel.type != interactions.ChannelType.PUBLIC_THREAD:
                                    ...
                                    try:    
                                            ...
                                            msg:interactions.Message = await channel.send(msg_txt, files=files.get(key))
                                            time.sleep(1)
                                            thread:interactions.Channel = await channel.create_thread(name=key, type=interactions.ChannelType.PUBLIC_THREAD, invitable=True, message_id=msg.id, reason=f"Store files of {key}")
                                            r = cur.execute(f"INSERT INTO thread_channel(thread_id, section_id, text_channel) SELECT DISTINCT '{thread.id}', section_id, '{channel.id}' FROM files WHERE section_name='{key}' and course_id='{ccon[0]}'")
                                            con.commit()
                                    except:
